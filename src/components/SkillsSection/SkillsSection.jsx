...
export default function SkillsSection({ style }) {
  ...
  return (
    <section className="relative" style={style}>
      ...
      <svg
        className="z-0 max-h-[40vh] w-full"
      ...
      <div className="relative min-h-[10vh]">
        {imagesWithRefs.map(({ src, ref }) => (
          <div
            key={src}
            className="absolute z-2 transform-[translate(-50%,-50%)] select-none [-khtml-user-select:none] [-moz-user-select:none] [-o-user-select:none]"
            ref={ref}
          >
            {/* TODO: i think mx-8 doesnt have any effect. check later */}
            <img className="mx-8 h-auto w-[clamp(30px,10vw,120px)]" src={src} />
          </div>
        ))}
      </div>
    </section>
